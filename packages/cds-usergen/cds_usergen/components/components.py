...
@solara.component
def SetUsernamePrefix(prefix: solara.Reactive[str], howMany: solara.Reactive[int]):
    with solara.Column():
        ...
        with solara.Columns(widths=[1, 2]):
            ...
            with solara.Row():
                ...
                solara.Button(
                    ...
                    icon=True,
                    on_click=lambda: email_domain.set(DEFAULT_EMAIL_DOMAIN),
                )
...
def GenerateButton(
    ...
    def create_users_action():
        ...
        users = auth0.create_user_file_from_list(usernames=usernames, prefix=prefix.value, passwords=passwords, domain=email_domain.value, create_file=False)
...
def JSONPreview(
    ...
    password: solara.Reactive[Optional[str]] = None,
    use_username_as_password: solara.Reactive[bool] = None,
    start_index: solara.Reactive[int] = None,
    end_index: solara.Reactive[int] = None,
):
    ...
    async def generate_preview():
        ...
        suffix_width = numDigits(howMany.value)
        start = start_index.value if start_index is not None else 1
        end = end_index.value if end_index is not None else howMany.value
        usernames = [getUsername(prefix.value, suffix_width, i) for i in range(start, end + 1)]
        passwords = None if use_username_as_password.value else [password_value] * len(usernames)
        prev = auth0.create_user_file_from_list(
            usernames=usernames,
            prefix=prefix.value,
            passwords=passwords,
        ...
        if password_value is None:
            ...
            [user.update({"password": password_value}) for user in prev]
